...
local function CheckStationHits()
    ...
    if table.type(bankHits) == 'array' then TriggerClientEvent('pn-bankrobbery:client:BankSecurity', -1, bankHits, false) end
end
...
RegisterNetEvent('pn-bankrobbery:server:setBankState', function(bankId)
    ...
    if bankId == 'paleto' then
        if Config.BigBanks['paleto']['isOpened'] or #(GetEntityCoords(GetPlayerPed(source)) - Config.BigBanks['paleto']['coords']) > 2.5 then
            return error(Lang:t('error.event_trigger_wrong', { event = 'pn-bankrobbery:server:setBankState', extraInfo = ' (paleto) ', source = source }))
        end
        Config.BigBanks['paleto']['isOpened'] = true
        TriggerEvent('pn-bankrobbery:server:setTimeout')
    elseif bankId == 'pacific' then
        if Config.BigBanks['pacific']['isOpened'] or #(GetEntityCoords(GetPlayerPed(source)) - Config.BigBanks['pacific']['coords'][2]) > 2.5 then
            return error(Lang:t('error.event_trigger_wrong', { event = 'pn-bankrobbery:server:setBankState', extraInfo = ' (pacific) ', source = source }))
        end
        ...
        if Config.SmallBanks[bankId]['isOpened'] or #(GetEntityCoords(GetPlayerPed(source)) - Config.SmallBanks[bankId]['coords']) > 2.5 then
            return error(Lang:t('error.event_trigger_wrong', { event = 'pn-bankrobbery:server:setBankState', extraInfo = ' (smallbank ' .. bankId .. ') ', source = source }))
        end
        Config.SmallBanks[bankId]['isOpened'] = true
        TriggerEvent('pn-bankrobbery:server:SetSmallBankTimeout', bankId)
    end
    TriggerClientEvent('pn-bankrobbery:client:setBankState', -1, bankId)
    robberyBusy = true
    Config.OnRobberyStart(bankId)
end)

RegisterNetEvent('pn-bankrobbery:server:setLockerState', function(bankId, lockerId, state, bool)
    ...
    if #(GetEntityCoords(GetPlayerPed(source)) - Config[bankType][bankId]['lockers'][lockerId]['coords']) > 2.5 then
        return error(Lang:t('error.event_trigger_wrong', { event = 'pn-bankrobbery:server:setLockerState', extraInfo = extraInfo, source = source }))
    end
    Config[bankType][bankId]['lockers'][lockerId][state] = bool
    TriggerClientEvent('pn-bankrobbery:client:setLockerState', -1, bankId, lockerId, state, bool)
end)

RegisterNetEvent('pn-bankrobbery:server:recieveItem', function(type, bankId, lockerId)
    local src = source
    local ply = QBCore.Functions.GetPlayer(src)
    if not ply then return end
    if type == 'small' then
        if #(GetEntityCoords(GetPlayerPed(src)) - Config.SmallBanks[bankId]['lockers'][lockerId]['coords']) > 2.5 then
            return error(Lang:t('error.event_trigger_wrong', { event = 'pn-bankrobbery:server:receiveItem', extraInfo = ' (smallbank ' .. bankId .. ') ', source = source }))
        end
        ...
        if WeaponChance ~= odd1 then
            if tier ~= 4 then
                ...
                exports['qb-inventory']:AddItem(src, 'security_card_01', 1, false, false, 'pn-bankrobbery:server:recieveItem')
            end
        else
            exports['qb-inventory']:AddItem(src, 'weapon_stungun', 1, false, false, 'pn-bankrobbery:server:recieveItem')
        end
    elseif type == 'paleto' then
        if #(GetEntityCoords(GetPlayerPed(source)) - Config.BigBanks['paleto']['lockers'][lockerId]['coords']) > 2.5 then
            return error(Lang:t('error.event_trigger_wrong', { event = 'pn-bankrobbery:server:receiveItem', extraInfo = ' (paleto) ', source = source }))
        end
        local itemType = math.random(#Config.RewardTypes)
        local tierChance = math.random(1, 100)
        ...
        if WeaponChance ~= odd1 then
            if tier ~= 4 then
                ...
                exports['qb-inventory']:AddItem(src, 'security_card_02', 1, false, false, 'pn-bankrobbery:server:recieveItem')
            end
        else
            exports['qb-inventory']:AddItem(src, 'weapon_vintagepistol', 1, false, false, 'pn-bankrobbery:server:recieveItem')
        end
    elseif type == 'pacific' then
        if #(GetEntityCoords(GetPlayerPed(source)) - Config.BigBanks['pacific']['lockers'][lockerId]['coords']) > 2.5 then
            return error(Lang:t('error.event_trigger_wrong', { event = 'pn-bankrobbery:server:receiveItem', extraInfo = ' (pacific) ', source = source }))
        end
        local itemType = math.random(#Config.RewardTypes)
        ...
        if WeaponChance ~= odd1 or WeaponChance ~= odd2 then
            if tier ~= 4 then
                ...
                exports['qb-inventory']:AddItem(src, 'cryptostick', 1, false, info, 'pn-bankrobbery:server:recieveItem')
            end
        else
            local chance = math.random(1, 2)
            local odd = math.random(1, 2)
            if chance == odd then
                exports['qb-inventory']:AddItem(src, 'weapon_microsmg', 1, false, false, 'pn-bankrobbery:server:recieveItem')
            else
                exports['qb-inventory']:AddItem(src, 'weapon_minismg', 1, false, false, 'pn-bankrobbery:server:recieveItem')
            end
        end
    end
end)

AddEventHandler('pn-bankrobbery:server:setTimeout', function()
    ...
    CreateThread(function()
        SetTimeout(60000 * 90, function()
            ...
            TriggerClientEvent('pn-bankrobbery:client:ClearTimeoutDoors', -1)
...
AddEventHandler('pn-bankrobbery:server:SetSmallBankTimeout', function(bankId)
    if robberyBusy or timeOut then return end
    timeOut = true
    CreateThread(function()
        SetTimeout(60000 * 30, function()
            ...
            TriggerClientEvent('pn-bankrobbery:client:ResetFleecaLockers', -1, bankId)
            timeOut = false
            robberyBusy = false
...
RegisterNetEvent('pn-bankrobbery:server:callCops', function(type, bank, coords)
    if type == 'small' then
        if not Config.SmallBanks[bank]['alarm'] then
            return error(Lang:t('error.event_trigger_wrong', { event = 'pn-bankrobbery:server:callCops', extraInfo = ' (smallbank ' .. bank .. ') ', source = source }))
        end
    elseif type == 'paleto' then
        if not Config.BigBanks['paleto']['alarm'] then
            return error(Lang:t('error.event_trigger_wrong', { event = 'pn-bankrobbery:server:callCops', extraInfo = ' (paleto) ', source = source }))
        end
    elseif type == 'pacific' then
        if not Config.BigBanks['pacific']['alarm'] then
            return error(Lang:t('error.event_trigger_wrong', { event = 'pn-bankrobbery:server:callCops', extraInfo = ' (pacific) ', source = source }))
        end
    end
    TriggerClientEvent('pn-bankrobbery:client:robberyCall', -1, type, coords)
end)

RegisterNetEvent('pn-bankrobbery:server:SetStationStatus', function(key, isHit)
    Config.PowerStations[key].hit = isHit
    TriggerClientEvent('pn-bankrobbery:client:SetStationStatus', -1, key, isHit)
    if AllStationsHit() then
        exports['qb-weathersync']:setBlackout(true)
        TriggerClientEvent('pn-bankrobbery:client:disableAllBankSecurity', -1)
        Config.OnBlackout(true)
        CreateThread(function()
            SetTimeout(60000 * Config.BlackoutTimer, function()
                exports['qb-weathersync']:setBlackout(false)
                TriggerClientEvent('pn-bankrobbery:client:enableAllBankSecurity', -1)
...
RegisterNetEvent('pn-bankrobbery:server:removeElectronicKit', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    exports['qb-inventory']:RemoveItem(src, 'electronickit', 1, false, 'pn-bankrobbery:server:removeElectronicKit')
    exports['qb-inventory']:RemoveItem(src, 'trojan_usb', 1, false, 'pn-bankrobbery:server:removeElectronicKit')
end)

RegisterNetEvent('pn-bankrobbery:server:removeBankCard', function(number)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    exports['qb-inventory']:RemoveItem(src, 'security_card_' .. number, 1, false, 'pn-bankrobbery:server:removeBankCard')
end)
...
QBCore.Functions.CreateCallback('pn-bankrobbery:server:isRobberyActive', function(_, cb)
    cb(robberyBusy)
end)

QBCore.Functions.CreateCallback('pn-bankrobbery:server:GetConfig', function(_, cb)
...
QBCore.Functions.CreateUseableItem('security_card_01', function(source)
    ...
    TriggerClientEvent('pn-bankrobbery:UseBankcardA', source)
end)
...
QBCore.Functions.CreateUseableItem('security_card_02', function(source)
    ...
    TriggerClientEvent('pn-bankrobbery:UseBankcardB', source)
end)
